...
train_clean_100_ds = load_dataset(
    "librispeech_asr", "all", split="train.clean.100", cache_dir="datasets")
train_clean_360_ds = load_dataset(
    "librispeech_asr", "all", split="train.clean.360", cache_dir="datasets")
train_other_500_ds = load_dataset(
    "librispeech_asr", "all", split="train.other.500", cache_dir="datasets")

# Concatenate the datasets
data = concatenate_datasets(
    [train_clean_100_ds, train_clean_360_ds, train_other_500_ds])
dev_data = load_dataset("librispeech_asr", "all",
                        split="test.clean+test.other", cache_dir="datasets")
...
def train(
    ...
    prompt_template_name: str = "phoenix",
):
    ...
    if ddp:
        ...
        print("WORLD SIZE", world_size, "NEW GRAD ACC STEPS",
    ...
    if use_lora or use_prompt_tuning:
        try:
            ...
            raise RuntimeError(f"Error loading the model: {
                               e}, CUDA Available: {torch.cuda.is_available()}")
    ...
    class CompositeDataCollator:
        def __init__(self, seq2seq_data_collator):
            self.asr_model = WhisperForConditionalGeneration.from_pretrained(
                f"openai/{MODEL_NAME}", cache_dir="cache/transformers").to(f"cuda:{local_rank}")
        ...
        def __call__(self, examples):
            if not examples:
                return []
            instruction = f"Perform error correction on the top {
                NUM_HYPOTHESES} outputs generated by an Automatic Speech Recognition (ASR) system. The ASR hypotheses, listed in order of their ASR posterior score, are as follows:\n"
            ...
            for example in examples:
                ...
                for i, beam_output in enumerate(beam_outputs):
                    hypothesis = f"<hypothesis{i + 1}>" + self.asr_processor.tokenizer._normalize(
                        self.asr_processor.decode(beam_output, skip_special_tokens=True)) + f"</hypothesis{i + 1}>\n"
                ...
                output = self.asr_processor.tokenizer._normalize(
                    example["text"])
            ...
            data = [generate_and_tokenize_prompt(
